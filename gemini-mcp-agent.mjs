...
const actions = {
  ...
  assign_archetypes: { description: `Only if phase is observing and the observation timer is zero. Assign every player exactly once, using all four distinct archetypes, and choose exactly two distinct expeditions from: ${EXPEDITION_IDS.join(', ')}. Treat every destination as equally viable: never default to Dark Cave plus Sunken Temple. The server applies a fair three-tale rotation so each destination appears equally often.`, args: { assignments: '[{playerId, archetype, evidence}]', expeditions: '[exactly 2 distinct expedition ids]' } },
...
function prompt(roomCode, telemetry, world) {
  ...
  return [
    ...
    'Every expedition is equally eligible. Do not repeatedly favour the Dark Cave and Sunken Temple; the server balances destination pairs across tales while you shape the roles and encounters around player behaviour.',
