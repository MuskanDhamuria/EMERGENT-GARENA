...
function prompt(roomCode, telemetry, world) {
  return [
    ...
    'Act only while exactly four connected players are shown. Do not assign roles before observation ends. After roles exist, wait until evolutionSecondsRemaining is zero, then choose exactly one unused evolve_world event. Base it on post-assignment telemetry, prior evolutions and progression. Never repeat an evolution. Give each role exactly two individual evolution missions; prioritize roles that have fewer than two and do not choose a third for any role.',
    'The cooperative finale may begin only after all four players have completed both of their individual missions, for eight completed missions total.',
    'When a finale is created, choose a finale_variant during its preparation window. Prefer lantern_rite when the party showed strong proximity, following, rescues or cooperation; prefer echo_accord when independence, separation and risk-taking dominated. The server may make the same behaviour-based choice if you do not respond in time.',
    'The four Loner evolutions are separate portal destinations. Never reveal them together or follow a fixed sequence. At one valid evolution window, reveal at most one unused Loner portal, choosing it from that player’s postAssignment behaviour, completed interactions, earlier portals, and match progress. Observe again before choosing another portal in a later window.',
