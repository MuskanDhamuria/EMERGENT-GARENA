...
export function createMcpRouter(world) {
  return async function handleMcpApi(request, response, pathname) {
    ...
    else if (pathname === '/api/mcp/world-evolution') { world.markGmActive(room); result = world.evolveWorld(room, payload.evolutionId, world.cleanText(payload.narration, '', 280), 'MCP Game Master'); }
    else if (pathname === '/api/mcp/finale') { world.markGmActive(room); const objective = world.createFinalObjective(room, 'MCP Game Master', { destinationEvolutionId: payload.destinationEvolutionId, roleEvolutionIds: payload.roleEvolutionIds, complicationId: payload.complicationId }); result = objective ? { ok: true, objective } : { ok: false, error: 'The finale requirements or proposed composition are not valid.' }; }
