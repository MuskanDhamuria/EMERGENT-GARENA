...
export function attachSocketGateway(io, world) {
  io.on('connection', (socket) => {
    ...
    socket.on('move', ({ x, z } = {}) => { const room = world.rooms.get(socket.data.roomCode), player = room && world.getPlayer(room, socket.id); if (player && ['observing', 'evolving', 'finale'].includes(room.phase)) { player.inputX = world.clamp(x, -1, 1); player.inputZ = world.clamp(z, -1, 1); if (Math.abs(player.inputX) > Math.abs(player.inputZ) && player.inputX) player.facing = player.inputX < 0 ? 'left' : 'right'; else if (player.inputZ) player.facing = player.inputZ < 0 ? 'up' : 'down'; } });
    ...
    socket.on('interact', ({ type, targetId, aimX, aimZ } = {}, callback = () => {}) => { const room = world.rooms.get(socket.data.roomCode), player = room && world.getPlayer(room, socket.id); const result = room && player ? world.interact(room, player, type, targetId, { aimX, aimZ }) : { ok: false, error: 'Join a room first.' }; callback(result); if (room) world.broadcastState(room); });
    socket.on('attack', (callback = () => {}) => { const room = world.rooms.get(socket.data.roomCode), player = room && world.getPlayer(room, socket.id); const result = room && player ? world.attackEncounter(room, player) : { ok: false, error: 'Join a room first.' }; callback(result); if (room) world.broadcastState(room); });
