...
assert.equal(player.moonShrine.lineFailed,false);assert.equal(player.moonShrine.pathStep,MOON_PATH.length-1);player.x=MOON_ALTAR.x;player.z=MOON_ALTAR.z;assert.equal(system.interact(room,player).kind,'complete');assert.equal(player.realm,'overworld');
