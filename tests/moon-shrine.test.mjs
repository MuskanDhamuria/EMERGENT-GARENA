...
const tolerance={...player,realm:'overworld',x:-9,z:12,interactions:{}};system.enter(room,tolerance);tolerance.moonShrine.pathStep=1;tolerance.x=8.4;tolerance.z=9;system.tick(room,tolerance,.001);assert.equal(tolerance.x,8.4,'a 28px path deviation remains within the 30px mistake range');tolerance.x=8.6;system.tick(room,tolerance,.001);assert.equal(tolerance.x,MOON_PATH[0].x,'a deviation beyond 30px resets the path');
