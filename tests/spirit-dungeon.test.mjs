...
const exit=dungeon.entities(loner).find((item)=>item.id==='dungeon-exit');loner.x=exit.x;loner.z=exit.z;assert.equal(dungeon.interact(room,loner,'dungeon-exit',exit.id).ok,true);assert.equal(loner.realm,'overworld');assert.deepEqual({x:loner.x,z:loner.z},{x:-3,z:10});assert.equal(loner.dungeon.phase,'COMPLETE');assert.equal(loner.dungeonCompletions,1);assert.equal(loner.completedEvolutions.has('spirit-portal-opens'),true);
